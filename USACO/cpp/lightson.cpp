...
int main()
{
    ...
    if (!switches[{1,1}].empty()){
        for (auto &i : switches[{1,1}]) arr[i.f][i.s] = true;
    }

    // set of visited
    // set of blocks to find
    uset<pii,hash_pair> already;
    uset<pii,hash_pair> noseen;

    noseen.insert({1,1});

    do {
        pii curr = *noseen.begin();
        already.insert(curr);

        // look for the others
        if (not switches[curr].empty()){
            for (auto &p : switches[curr]) arr[p.f][p.s] = true;
        }

    } while (not noseen.empty())
