...
vec<cow> cows;
uset<int> distx, disty;

int prefix[2500][2500];

void solve(){
    int numx = distx.size() + 1, numy = disty.size() + 2;
...
int main()
{
    ...
    /*
        let x be number of distinct x's
        let y be number disitncy y's

        array dim (x+1, y+1)
        accounts for left side
        prefix sum

        O(nlogn + n^2 + 2n^2) *should* work within the time constraints
    */
    solve();
