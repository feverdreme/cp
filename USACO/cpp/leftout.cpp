...
int n;
bool carr[1000][1000];
const size_t maxn2 = 1000*1000;
int countr = 0;
int countl = 0;

bool solve (int &i, int &j, bool minmax){ // if true then minimize, if flase then maximize
    bool arr[1000][1000];
    copy(&carr[0][0], &carr[0][0] + maxn2, &arr[0][0]);
...
int main()
{
    ...
    // try to count
    // guess the one that doesnt work
    pii ans;
    F0R(i,n) F0R(j,n) {
        if (solve(i,j,true) || solve(i,j,false)){
            ans = {i,j};
            break;
        }
    }
