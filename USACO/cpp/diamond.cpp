...
int main()
{
    ...
    rep(n) cin >> diamonds[i];

    sort(diamonds.begin(), diamonds.end());
    // calculate the sets of all of them
    ...
    for (int ctr=0; ctr<n; ctr++){
        ...
        while (ptr1 >= 0){
            if (diamonds[ctr] - diamonds[ptr1] > k) break;
            ptr1--;
        }
        while (ptr2 < n){
            if (diamonds[ptr2] - diamonds[ctr] > k) break;
            ptr2++;
        }

        cases[ctr] = uset<ll>(diamonds.begin() + ptr1, diamonds.begin() + ptr2 + 1);
    }

    // solve for all the ptrs;
