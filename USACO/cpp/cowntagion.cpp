...
uset<int> visited;
ll dfs(node& farm){
    visited.insert(farm.id);
    int numedges = farm.size();
    if (numedges == 1) return 0;

    // now dfs them
    int numdouble = (int)ceil(log2(numedges));
    ll localans = numdouble + numedges - 1;

    for (int e : farm.edges){
        if (visited.find(e) == visited.end() && e != 0) localans += dfs(farms[e]);
    }
