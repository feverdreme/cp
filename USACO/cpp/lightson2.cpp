...
void dfs(pii p){
    ...
    for (auto &i : r.switches){
        rooms[i.f][i.s].on = true;

        FOR(i,-1,2) FOR(j,-1,2) {
            if (!(i == 0 ^ j == 0)) continue;
            if (x + i < 0 || x + i > 100 || y + j < 0 || y + j > 100) continue;

            if (rooms[x+i][y+j].seen) {
                ok = true;
                goto en;
            }
        }
...
int main()
{
    ...
    rep(m){
        cin >> x >> y >> a >> b;
        rooms[x][y].switches.pb({a,b});
    }

    // start from (1,1) and dfs, everytime you do a new light thing dfs there and see
    dfs({1,1});
