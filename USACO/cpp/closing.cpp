...
bool dfs(){
    ...
    stack<int> stk;
    //find the first member that isnt closed

    for(int i=1;i<=N;i++){
        // cout << (rem.find(i) == rem.end()) << sp << i << aendl;
    ...
    // do the dfs
    int curr;
    while(stk.size()){ // while it has a size
        curr = stk.top();
        traversed.insert(curr);
        stk.pop();

        for (auto &e : edges[curr]){
            if (rem.find(e) == rem.end() && traversed.find(e) == traversed.end()){
                stk.push(e);
                traversed.insert(e);
            }
        }

        if (traversed.size() == n) return true;
    }
