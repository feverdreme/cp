...
map<char,int> getchars(string &a){
    map<char,int> chars = temp;

    for (auto &i : a) chars[i]++;
...
int main() {
    ...
    cin >> n;
    words.resize(n);
    rep(n) cin >> words[i].f >> words[i].s;

    for (auto &i: letters) temp[i] = 0;

    // add everythign to the count
    map<char,int> a, b, o;
    auto ans = temp;
    for (auto &i : words){
        a = getchars(i.f);
        ...
        for (auto &i : letters) ans[i] += max(a[i], b[i]);
    }

    for (auto &i : letters) cout << ans[i] << endl;
