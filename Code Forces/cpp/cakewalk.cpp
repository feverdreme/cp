...
struct pii {
    ...
    pii();
    pii(int a, int b): aa(a), bb(b){}

    int operator[](size_t ind){
        return ind ? bb : aa;
    }

    bool operator==(pii other){
        return (other[0] == aa && other[1] == bb);
    }
};

pii find_cake(pii pos){
    queue<pii> q;
    ...
    while(!q.empty()){
        ...
        if (curr[0] >= w || curr[1] >= h) continue;

        // cout << curr[0] << ' ' << curr[1] << endl << arr[curr[0]][curr[1]] << endl;

        if (arr[curr[0]][curr[1]] == '*') return curr;

        if (curr[0] < w-1) q.push({curr[0]+1, curr[1]});
        if (curr[1] < h-1) q.push({curr[0], curr[1]+1});

        if (curr[0] == w - 1 && curr[1] == h - 1)
            return {-1, -1};
    }
}
...
int main()
{
    ...
    while(true){
        pii newpos = find_cake(pos);

        if (newpos == pii({-1,-1})) break;
        ans++;
        pos = newpos;
    }
