...
int main()
{
    ...
    while (t--){
        cin >> s;
        n = s.size();
        string scpy (s.begin(), s.end());

        // we find all palindome pivots, and then replace them with characters that dont exist
        uset<char> schars (s.begin(), s.end());
        uset<char> nin;
        // find the chars that arent there
        rep(26) if (schars.find(alphabet[i]) == schars.end()) nin.insert(alphabet[i]);
        ...
        while (changed){
            ...
            for (int i=1; i<n; i++){
                ...
                if (s[i] == s[i-1]){
                    if (nin.empty()){
                        // find something that isnt teh letter
                        for (char c : alphabet){
                            if (c != s[i-1]){
                                s[i-1] = c;
                                break;
        ...
        // go through all and find the diffs
        int ans = 0;
        for (int i=0; i<n; i++){
            ans += s[i] != scpy[i];
        }
