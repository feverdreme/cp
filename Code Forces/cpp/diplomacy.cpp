...
multimap<int,int>::iterator findm(multimap<int,int> &m, int v){
    for (auto it = m.begin(); it != m.end(); it++){
        if (it->second == v) return it;
    }
}
...
int main()
{
    ...
    while(t--){
        ...
        rep(m){
            ...
            F0R(j,k) cin >> aval[i][j];
        }

        vi sized(m);
        iota(sized.begin(), sized.end(), 0);
        
        sort(sized.begin(), sized.end(), [&aval](int a, int b){
            return aval[a].size() <= aval[b].size();
        });

        // now keep a running set => umap{person, times} and if you can't scan through properly then impossible
        vi ans (m);

        multimap<int,int> tally;
        map<int,int> v2k;
        FOR(i,1,n+1) {
            tally.insert({0, i});
            v2k[i] = 0;
        ...
        for (auto &day : sized){
            ...
            if (able.size() == 1){
                ...
                if (v2k[selected] > n2){
                    v2k.erase(selected);
                    tally.erase(findm(tally, selected));
                }
            } else {
                // find the one with the least value rn
                auto least = *tally.begin();
                int selected = least.second;
