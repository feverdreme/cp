...
void solve(vi& li, vi &gi, vi &a, vi &b){
    // go through until everything is done
    vec<pii> ops;
    ...
    while (true){
        ...
        ops.pb({gi[gptr], li[lptr]});

        if (a[gi[gptr]] == b[gi[gptr]]) gptr--;
        if (a[li[lptr]] == b[li[lptr]]) lptr--;

        if (gptr < 0 || lptr < 0) break;
    }
...
int main()
{
    ...
    while(t--){
        ...
        // check if the differecnes are even
        ...
        // the left has to be a < b and the right has to be a > b
        vi li;
        vi gi;
